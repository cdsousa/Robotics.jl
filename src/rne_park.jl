...
function rne_park_forward(q, dq, ddq, g, Tdh_inv, S, ifunc=identity)
    ...
    # Forward
    for i in 1:dof
        V[i+1] = ifunc(Adj(Tdh_inv[i], V[i])) + ifunc(S[i]*dq[i])
        V[i+1] = ifunc(V[i+1])

        dV[i+1] = ifunc(S[i]*ddq[i]) + ifunc(Adj(Tdh_inv[i], dV[i])) +
                ifunc(adj(ifunc(Adj(Tdh_inv[i], V[i])), ifunc(S[i]*dq[i])))
        dV[i+1] = ifunc(dV[i+1])
    end
...
function rne_park(q, dq, ddq, Tdh_inv, S, g, L, l, m, ifunc=identity)
    V, dV = rne_park_forward(q, dq, ddq, g, Tdh_inv, S, ifunc=identity)
    tau = rne_park_backward(Tdh_inv, S, L, l, m, V, dV, ifunc=identity)
    tau
end
